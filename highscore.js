...
function getHighScoreTable() {
    ...
    for (var i = 0; i < 10; i++) {
		...
		var playerscore = getCookie(player);
        // If the cookie does not exist exit from the for loop
		if (!playerscore)break;
        // Extract the name and score of the player from the cookie value
		var username = playerscore.split("~")[0];
		var userscore = playerscore.split("~")[1];
        // Add a new score record at the end of the array
        table.push(new ScoreRecord(username, userscore));
    }
...
//
function setHighScoreTable(table,uid) {
	var cookie = null;
    for (var i = 0; i < 10; i++) {
        ...
        // Contruct the cookie name
        setCookie("player" + i.toString(),table[i].name +"~"+table[i].score);
